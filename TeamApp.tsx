...
const TeamApp: React.FC<TeamAppProps> = ({ onLogout, theme, toggleTheme, currentUser }) => {
  ...
  const handleUpdateTask = async (updatedTask: Task) => {
    ...
    if (originalTask) {
        ...
        const updatedAssigneeIds = updatedTask.assigneeIds || [];
        
        // Find assignees that are in updated but not in original
        const newAssignees = updatedAssigneeIds.filter(id => !originalAssigneeIds.includes(id));
