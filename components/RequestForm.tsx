...
const RequestForm: React.FC<RequestFormProps> = ({ onTaskAdded, tasks }) => {
  const [formData, setFormData] = useState({
    ...
    requestDate: new Date().toISOString(), // Keep for initial state, but submission will use fresh timestamp
    dueDate: '',
    requesterName: '',
    department: '',
    committee: '',
  ...
  useEffect(() => {
    ...
    if (storedEmail) {
      setFormData(prev => ({ ...prev, requesterEmail: storedEmail }));
    }
  }, []);
  ...
  const validate = (data = formData) => {
    ...
    if (!data.dueDate) {
        ...
        const requestDateOnly = new Date(); // Use current date for comparison
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-8" noValidate>
        <FormSection title="ข้อมูลเบื้องต้น">
            ...
            <InputField label="4. ชื่อ-สกุล ผู้สั่งงาน *" name="requesterName" value={formData.requesterName} onChange={handleChange} onBlur={handleBlur} error={errors.requesterName} required placeholder="เช่น นายสมชาย ใจดี"/>
            <InputField label="5. สังกัดฝ่าย/ส่วน *" name="department" value={formData.department} onChange={handleChange} onBlur={handleBlur} error={errors.department} required placeholder="เช่น ฝ่ายมาตรฐานการบัญชี"/>
            ...
            <InputField label="7. อีเมล *" name="requesterEmail" type="email" value={formData.requesterEmail} onChange={handleChange} onBlur={handleBlur} error={errors.requesterEmail} required placeholder="example@tfac.or.th"/>
